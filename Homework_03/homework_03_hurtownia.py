...
Po wprowadzeniu odpowiedniej komendy, aplikacja zachowuje się w unikalny sposób dla każdej z nich:
    ...
    zakup - Program pobiera nazwę produktu, cenę oraz liczbę sztuk. Produkt zostaje dodany do magazynu, jeśli go
        nie było. Obliczenia są wykonane odwrotnie do komendy "sprzedaz". Saldo konta po zakończeniu operacji „zakup” nie może być ujemne.
    ...
    przegląd - Program pobiera dwie zmienne „od” i „do”, na ich podstawie wyświetla wszystkie wprowadzone akcje zapisane
        ...
        przegląd od początku lub/i do końca. Jeżeli użytkownik podał zmienne spoza zakresu, program powinien o tym poinformować i wyświetlić liczbę zapisanych komend (żeby pozwolić użytkownikowi wybrać odpowiedni zakres).
...
initital_message="Witaj. Wybierz numer komendy z dostępnych dla twojego magazynu:"
run_operation = True
while run_operation:
    print(initital_message)
    for keys, values in available_prompts.items():
       print(f'{keys}: {values}')
    choice = int(input('Podaj numer komendy z listy powyżej: '))
    match choice:
        case 1:
            amount = float(input('Podaj kwotę konta: ') or "200")
            add_deduct = input('Wpisz:\n "+" jeśli chcesz dodać saldo\n "-" jeśli chcesz odjąć saldo:\nWybierz + lub -:') or "+"
            if add_deduct == "+":
